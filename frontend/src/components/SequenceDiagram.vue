...
export default {
  ...
  created () {
    ...
    for (let i = 0; i < this.dataList.length; ++i) {
      ...
      let line = Object.assign({}, this.dataList[i])
      this.rowList.push(line)

      // undefined === undefined --> true
      if (line['from'] === line['to']) {
        // -------------自循环，从A->A-----------------
        let columnIndex = this._ensureColumnIndex(line['name'])
        ...
        // -------------正常信息，从A->B-----------------
        // 记录列的信息，并返回列的索引位置
        let fromColumnIndex = this._ensureColumnIndex(line['from'])
        // 记录当前行
        ...
        // to相关信息
        let toColumnIndex = this._ensureColumnIndex(line['to'])
  ...
  methods: {
    ...
    _ensureColumnIndex (columnName) {
      ...
      let idx = this.columnList.length
    ...
    _generateColumnSegment (columnItem) {
      ...
      for (let i = 0; i < columnItem['rowNumbers'].length; ++i) {
    ...
    reCalculateLinePosition () {
      ...
      // segment block
      let segBlockDom = document.querySelector('[name=subsys0] .segment-block')
      if (!segBlockDom) {
        return
      }
      // segment block宽度
      let segBlockWidth = segBlockDom.clientWidth
      // 列的高度
      let columnBodyDom = document.querySelector('[name=subsys0] .bodybox')
      // 清空原样式
      this.lineStyles.length = 0
      this.labelStyles.length = 0
      for (let i = 0; i < this.rowList.length; ++i) {
        let line = this.rowList[i]
        // let lineDom = this.$refs.line[i]
        let width = 0
        let top = 0
        let left = 0
        // calculate top
        let segCount = this.rowList.length + 1
        let avgSegheight = columnBodyDom.offsetHeight / segCount
        let columnBodyTop = columnBodyDom.offsetTop

        // A->A类型
        if (line['fromColumnIdx'] === line['toColumnIdx']) {
          let columnDom = this.$refs.subsys[line['fromColumnIdx']]
          ...
          // A->B OR B->A
          // 开始列的dom
          let fromDom = this.$refs.subsys[line['fromColumnIdx']]
          // 结束列的dom
          let toDom = this.$refs.subsys[line['toColumnIdx']]
